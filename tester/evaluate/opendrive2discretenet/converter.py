...
from opendrive2discretenet.plane_elements.plane import (
    ParametricLane,
    ParametricLaneBorderGroup,
)
from opendrive2discretenet.plane_elements.plane_group import ParametricLaneGroup
from opendrive2discretenet.plane_elements.border import Border
...
class OpenDriveConverter:
    ...
    def _create_outer_lane_border(lane_borders, lane, coeff_factor) -> Border:
        """
        创建一个车道的外车道边界，内侧边界已经保存在lane_borders中，因为他是内侧相邻车道的外边界

        Args:
          lane_borders: 之前计算的内侧车道的边界
          lane: 应该被计算外边界的车道
