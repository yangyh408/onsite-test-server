...
from opendrive2discretenet.opendriveparser.elements.eulerspiral import EulerSpiral
...
class Geometry(abc.ABC):
    ...
    # 私有属性，不能更改
    @property
    def start_position(self) -> ndarray:
...
class Line(Geometry):
    ...
    """
    def __init__(self, start_position, heading, length):
...
class Arc(Geometry):
    ...
    def calc_position(self, s_pos):
        ...
        a = 2 / c * np.sin(s_pos * c / 2)  # 计算等腰三角形底边等于多少
        alpha = (np.pi - s_pos * c) / 2 - hdg  # 计算等腰三角形底边与水平的夹角

        dx = -1 * a * np.cos(alpha)  # 参考线在x方向上的变化
        dy = a * np.sin(alpha)  # 参考线在y方向上的变化

        pos = self.start_position + np.array([dx, dy])  # 圆结束时参考线的坐标
        tangent = self.heading + s_pos * self.curvature  # 圆结束时的切线方向
...
class Poly3(Geometry):
    ...
    def calc_position(self, s_pos):
        ...
        需要在计算位置时，将参考坐标系转至参考系坐标系st中，即t(s)=a + b*s + c*s**2 + d*s**3
        通常情况下，t/s坐标系与v/u坐标系的起始位置与起始方向是一致的
        ...
        dCoeffs = coeffs[1:] * np.array(np.arange(1, len(coeffs)))  # 求导
        tangent = np.polynomial.polynomial.polyval(s_pos, dCoeffs)  # 带入
...
class ParamPoly3(Geometry):
    ...
    def calc_position(self, s_pos):
        ...
        # 基于参数方程计算xy坐标
        u = np.polynomial.polynomial.polyval(pos, coeffsU)
        v = np.polynomial.polynomial.polyval(pos, coeffsV)

        # 参考线st坐标系转惯性xy坐标系
        xrot = u * np.cos(self.heading) - v * np.sin(self.heading)
        yrot = u * np.sin(self.heading) + v * np.cos(self.heading)
