...
from opendrive2discretenet.opendriveparser.elements.geometry import (
    Geometry,
    Line,
    Spiral,
    ParamPoly3,
...
class PlanView:
    ...
    def calc(self, s_pos: float) -> Tuple[np.ndarray, float]:
        ...
        """
        # 这里提供两种计算坐标方式：一种是预先计算过一下坐标，可以理解为有一定的样本，然后对样本进行插值得到所求的坐标和方向角
        # 第二种是根据每一段的geometry的方法计算
    ...
    def interpolate_cached_values(self, s_pos: float) -> Tuple[np.ndarray, float]:
        """
        通过插值计算s_position对应的（x,y）和转向角
        self._precalculation是ndarray，列数为4，分别为：s,x,y,rot
