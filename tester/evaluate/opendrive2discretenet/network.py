...
模块包含Network，它可以加载一个openDrive对象，然后导出laneLets。首先，将路网表示为参数化的三车道线"""

from opendrive2discretenet.discrete_network import *
from opendrive2discretenet.opendriveparser.elements.opendrive import OpenDrive

from opendrive2discretenet.utils import encode_road_section_lane_width_id
from opendrive2discretenet.converter import OpenDriveConverter


def convert_to_new_lanelet_id(old_lanelet_id: str, ids_assigned: dict) -> int:
    """
    将旧的laneLet id（格式：501.1.-1.-1）转换成新的，更简单的（100，101 etc.）
    ...
    为此，可以从100开始，连续地将数字赋值给old_lanelet_id字符串。将赋值保存在dict中，并将其作为ids_assigned传递给函数。
    ...
    if old_lanelet_id in ids_assigned.keys():
        new_lanelet_id = ids_assigned[old_lanelet_id]  # 该id已经被添加到ids_assigned中
    else:
        try:
            new_lanelet_id = max(ids_assigned.values()) + 1  # 未被添加时
        except ValueError:
            new_lanelet_id = starting_lanelet_id  # 初始时，字典为空
        ids_assigned[old_lanelet_id] = new_lanelet_id  # 将未添加的id加入
...
class Network:
    """
    表示参数化车道网络，带有链接索引，其中储存了参数车道间的邻域关系
...
class LinkIndex:
    ...
    找到文件中所有所有的连接关系，将所有内容记录为后驱，前驱可以通过反向搜索找到
    """
        ...
        for road in opendrive.roads:
            for lane_section in road.lanes.lane_sections:
                for lane in lane_section.allLanes:
                    ...
                    -------------------------------------------------------------------------
                    elif对该Road中最后一个section进行单独讨论，由于Road最后一个section仅可能被下一个Road/Junction后继
                    ...
                    -------------------------------------------------------------------------
                    elif对该Road中第一个section进行单独讨论，由于Road第一个section仅可能被上一个Road/Junction前驱
    ...
    def add_link(self, parametric_lane_id, successor, reverse: bool = False):
        """
        记录交叉口中每个连接路的前驱和后驱之间的关系，数据格式为字典形式，前驱key，后驱为value
        Args:
          parametric_lane_id:
          successor:
