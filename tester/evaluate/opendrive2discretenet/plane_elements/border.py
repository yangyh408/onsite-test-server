...
class Border:
    ...
    def __init__(self, ref_offset: float = 0.0):
        ...
        self.width_coefficient_offsets = []  # 里面存放不同geometry的起点位置
    ...
    def _get_width_index(self, s_pos: float, is_last_pos: bool) -> int:
        ...
        return next(
            (
                ...
                for n in self.width_coefficient_offsets[::-1]  # [0,30,50,70,95,120],s=75,
                if (
                    (n <= s_pos and (not is_last_pos or s_pos == 0))
                    or (n < s_pos and is_last_pos)
                )  # 构建了一个list，这个list里面包括了超出车道的坐标索引
            ),
            len(self.width_coefficient_offsets),   # 超出部分索引按width_coefficient_offsets的长度计算
        )
    ...
    # NOTE: might be more efficient to calculate each border once
    # instead of recalculating them over and over.
    # least recently used 策略释放缓存
