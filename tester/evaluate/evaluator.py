...
def evaluating_multiple_scenarios(trajectory_path: str, scenario_path: str, interval: int = 5) -> tuple:
    ...
    dd_y=[]
    turn_a=[]
    trajectory_list = list(os.listdir(scenario_path))
    for item in trajectory_list:
        ...
        if item[0] != '.' and item[-3:]!='.py' and item[0] != '_':
            all_scenario.append(item)
...
def recording(result: tuple, output: str, detail_output: str) -> None:
    ...
    result_dataframe = pd.DataFrame(result_array.T,
                                    columns=['Scenario', 'Safety', 'Efficiency', 'Comfort', 'Total', 'mean_ttc',
                                             'oppo_lane_rate', 'out_lane_rate', 'run_red', \
                                             'done_ornot', 'Mean_v', 'Distance_todest', 'a_x', 'd_x', 'a_y', 'd_y',
                                             'aa_x', 'dd_x', 'aa_y', 'dd_y', 'turn_a'])
    result_dataframe0 = pd.DataFrame(np.insert(result_dataframe.values, len(result_dataframe.index),
                                               values=["Mean", round(mean(result_array[1, :].astype(float)), 2),
                                                       round(mean(result_array[2, :].astype(float)), 2),
                                                       round(mean(result_array[3, :].astype(float)), 2),
                                                       round(mean(result_array[4, :].astype(float)), 2),
                                                       mean(result_array[5, :].astype(float)),
                                                       mean(result_array[6, :].astype(float)),
                                                       mean(result_array[7, :].astype(float)),
                                                       mean(result_array[8, :].astype(float)), \
                                                       mean(result_array[9, :].astype(float)),
                                                       mean(result_array[10, :].astype(float)),
                                                       mean(result_array[11, :].astype(float)),
                                                       mean(result_array[12, :].astype(float)),
                                                       mean(result_array[13, :].astype(float)),
                                                       mean(result_array[14, :].astype(float)), \
                                                       mean(result_array[15, :].astype(float)),
                                                       mean(result_array[16, :].astype(float)),
                                                       mean(result_array[17, :].astype(float)),
                                                       mean(result_array[18, :].astype(float)),
                                                       mean(result_array[19, :].astype(float)),
                                                       mean(result_array[20, :].astype(float))], axis=0))
    result_dataframe0.columns = result_dataframe.columns
    result_dataframe1 = pd.concat([result_dataframe0, note], axis=1)
    result_dataframe1.columns = ['Scenario', 'Safety', 'Efficiency', 'Comfort', 'Total', 'mean_ttc', 'oppo_lane_rate',
                                 'out_lane_rate', 'run_red', \
                                 'done_ornot', 'Mean_v', 'Distance_todest', 'a_x', 'd_x', 'a_y', 'd_y', 'aa_x', 'dd_x',
                                 'aa_y', 'dd_y', 'turn_a', 'Note']
    ...
    result_dataframe0 = pd.DataFrame(np.insert(result_dataframe.values, len(result_dataframe.index),
                                               values=["Mean", round(mean(result_array[1, :].astype(float)), 2),
                                                       round(mean(result_array[2, :].astype(float)), 2),
                                                       round(mean(result_array[3, :].astype(float)), 2),
                                                       round(mean(result_array[4, :].astype(float)), 2)], axis=0))
    result_dataframe0.columns = result_dataframe.columns
    result_dataframe1 = pd.concat([result_dataframe0, note], axis=1)
...
if __name__ == "__main__":
    ...
    if mode == 1:
        ...
        Output =r"C:\Users\Administrator\Desktop\评分inter"
        record = True
        score_result = evaluating_multiple_scenarios(Trajectory_path, Scenario_path, Interval)
        if record:
            #print(score_result)
            #pd.DataFrame(score_result).to_csv(r"C:\Users\Administrator\Desktop\评分inter\1.csv")
            recording(score_result, Output,Output)
